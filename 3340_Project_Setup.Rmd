...
```
Please set working directory to where you have your dataset and upload the csv file with the correct path. 
...
```
In this first chunk of code, we are removing irrelevant data columns and useless data entries. We are also removing rows with US territories. 


```{r}
add_case(df_4, 
YearStart = 2012, LocationDesc = "Alabama", Topic = "Obesity / Weight Status", Question = "Percent of adults aged 18 years and older who have obesity", Data_Value = 29.999, Data_Value_Footnote_Symbol = "", Low_Confidence_Limit = 23.999, High_Confidence_Limit = 33.499, Sample_Size = 1255, Total = "", Age.years. = "", Education = "", Gender = "", Income = "", Race.Ethnicity = "Asian", GeoLocation = "(32.84057112200048, -86.63186076199969)", LocationID = 1, StratificationCategory1 = "Race/Ethnicity", Stratification1 = "Asian") -> df_5
filter(df_5, Income != "Data not reported") -> df_6

df_6["311", "Sample_Size"] <- 1974 
```
This next chunk adds our new data point. This is to ensure that every dataset is unique from other groups in the class. Here we have added a new Asian demographic into income(where 1255 was previously unavailable. 
...
df_total <- df_10
```
As explained in the introduction, we have 6 levels of income in our data. This code assigns each income range a value instead of using the range (for example, "$25,000 - $34,999") every time. 
...
filter(df_q, IncomeID != "") -> df_prime #Erases blank IncomeID values
