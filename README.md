...
We then need to ensure that we have reasonable magnitudes of our coefficients in each study.  To validate this we used the method of robust statistics.  This is typically done when the model cannot be transformed to follow the assumptions.  Robust statistics attemps to reduce the impact of any influencial values as well as violations.  To perform this, R has a built in "rlm" function; however, the library "MASS" needs to be called for this to work.  We essentially compared the "lm" models with the "rlm" models to  see if there was any significant difference between the two.  Supposely, they should be the same (in fact they are) which means the assumptions of a linear model have not been violated.
