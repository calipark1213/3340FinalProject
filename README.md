...
We will be defining the consumption of fruit and vegetables as our two main predictor variables, x<sub>1</sub> and x<sub>2</sub> respectively (i.e. they will be our independent variables), and fit a linear regression model to determine if it has an affect on obesity rates. Before we fit the model, we begin by normalizing the predictor variables by taking the mean and subtracting it from each point to allow easier interpretation and to reduce variability.  To begin, we started off with the 'lm' command which will produce a linear model between obesity rates (y), and the number of people who consume less than one fruit per day (x1) and less than on vegetable a day (x2).  This will be our main method for analyzing the data.  The first technique used was least-squares estimation.  This is when we take the acutal values and subtract it from the fitted values (evaluated at the given slope/intercept B1 and B0).  We take the sum and square these values (S), take the partial derivatives of B0 and B1 respectively, and solve for B1 and B0.  These then generate the least squares estimators B0 and B1 that will make S as small as possible.  

A similar procedure is used to observe more than one variable at a time where there is one dependent variable (y) and k predictors.  Least squares estimation was then  performed once more by finding the values of B0, B1,...Bk that minizmie the sum of squares
...
Considering that our multiple linear model is the most important for further analysis, and has the largest _R<sup>2</sup>_, we double checked the model by transforming our data into matricies and performing least squares estimations. Lines 104 - 118 of our Rmd give the following matrix:
